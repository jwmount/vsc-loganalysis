QClass = Class.new
class QClass
  define_method :aqps do
    "%.3f" %  (@count/(Time.new - @started_at)).to_f
  end
  define_method :begin do
#    FILTER_NAME = "Filter_count"
    FILTER_NAME = $0
    QRY_FILE = ARGV[0] ||= "qry_file.txt"                               #default input file
    QRY_LIMIT = ARGV[1] ||= 0                                           #limit queries to read
  ...
  define_method :qry_file do
    QRY_FILE
  end
  define_method :inc_count do
      @count = (@count || 0) + 1
  end
  define_method :count_asString do
      @count.to_s
  end
  define_method :limit? do 
    inc_count
    if @count > QRY_LIMIT.to_i
      wrap_up
      puts "Terminated, reached limit at #{QRY_LIMIT}"
      Kernel.exit 99
    end
  end
  define_method :log  do |l|
    @log.write l
  end
  ...
  define_method :progress? do
    puts FILTER_NAME + ": " + @count.to_s + "\trecords @\t" +  aqps + " queries/sec." if @count.modulo(1000)  == 0
  end
  define_method :wrap_up do
    ...
    @log.write( "Input file count: \t\t\t\t\t" + "%d" % @count.to_s + "\n")
    @log.write( "Total seconds\t\t\t\t\t\t" + "%.3f" %  (completed_at - @started_at).to_f + "\n" )
    @log.write( "Average records per second:\t\t\t" + "%.3f" %  (@count/(completed_at - @started_at)).to_f +  "\n" )
    @log.write( "__END__\n")
